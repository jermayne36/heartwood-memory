...
3. Select `heartwood-rotation-demo`, review its command and environment names,
   and start it.
4. Approve the first-run workspace server trust prompt.
...
VS Code stores the enable/disable and trust choices separately from the shared
workspace file. An organization can centrally restrict MCP access; validate that
policy in the actual design-partner environment before promising this venue.
...
If Copilot is not installed, not signed in, or organization policy disables MCP,
run the same demo through the Codex/Claude/Ollama route harness. The transcript
still proves Heartwood's model-independent memory, policy, explain, provenance,
and audit behavior. Keep the VS Code configuration as deployment evidence; do
not represent it as a successful constrained-organization validation.
