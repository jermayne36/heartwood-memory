...
The production Ed25519 public key and its SHA-256 fingerprint are pinned in
[`heartwood-release-public-key-v1.txt`](heartwood-release-public-key-v1.txt) and
[`heartwood-release-public-key-v1.sha256`](heartwood-release-public-key-v1.sha256).
Each tagged CI release publishes the signed manifest and the same public key
beside the GitHub release, while the wheel remains on PyPI. A customer can
verify the downloaded wheel without an account or support contact:
...
python3.11 -m pip install --quiet --target "$VERIFY_DIR/site" "cryptography>=42" "$VERIFY_DIR"/heartwood_memory-*.whl && PYTHONPATH="$VERIFY_DIR/site" python3.11 -m heartwood.release_verify --manifest "$VERIFY_DIR/manifest.json" --public-key "$VERIFY_DIR/public-key.txt" --public-key-fingerprint "sha256:64b76c1534d0da6980446f945c0d422a046089b17c5b2558808ccd0e79d9d8a6" --artifact "$VERIFY_DIR"/heartwood_memory-*.whl
```

Expected output starts with `VERIFIED heartwood_memory-X.Y.Z-...whl` and its
SHA-256 digest. The verifier requires the externally pinned fingerprint; a key
downloaded from the same release is not trusted merely because it is
self-consistent with that release's manifest.

The Rule-45-approved production root is registered in CI as
`HEARTWOOD_RELEASE_SIGNING_KEY_B64`, which makes `manifest_key_source=env`.
The workflow fails if the secret is absent or if its derived public key differs
from the pinned v1 public key. Local development without the secret still uses
an ephemeral key. Never create, rotate, replace, revoke, or delete a signing key
during a release without a credential-change approval and rollback plan.

### Compromise, revocation, and rollback

If the v1 root may be compromised, halt tag releases immediately and mark the
incident in the release channel. Do not overwrite the v1 public-key files or
silently re-sign historical artifacts. Preserve each affected release and its
original verification assets as evidence, publish a revocation notice naming
the v1 fingerprint and the last known-good release, and obtain a new Rule 45
approval before creating a successor root.

After approval, publish a new versioned public-key and fingerprint pair, update
CI to require the successor fingerprint, and issue a new package version whose
manifest is signed only by the successor. Auditors retain both fingerprints:
v1 for historical releases through the declared cutoff, and the successor for
later releases. If cutover validation fails before any public tag is released,
disable the release job and restore the last approved CI secret; never fall back
to ephemeral signing for a production tag. The blast radius of a compromise is
publisher identity for every artifact signed by the affected root, not package
hash integrity alone.
